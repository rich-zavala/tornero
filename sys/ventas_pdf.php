...
if($tipo == "f"){ // Es factura
	...
	if(strlen($cliente_data[5]) > 0 ){ $cliente_data[5] = "COLONIA: ".$cliente_data[5];}
	$datos .= "CALLE: ".$cliente_data[2].$cliente_data[3].$cliente_data[4]."\n";	
	$datos .= $cliente_data[5]." CP: ".$cliente_data[0]."\n";
  ...
  foreach($secs as $k => $v){
  	...
  	if($k == "Tabla_de_Productos"){	
  		...
  		while($r = mysql_fetch_assoc($qq)){
  			...
  			$pdf->Cell($w[4],15,number_format($r[precio],2),0,0,"C"); //Precio
  ...
  $pdf->SetXY(150,50);
  $pdf->Cell(280,13,"La Casa Del Tornero S.A. De C.V.",0,0,"C");  
  ...
  $pdf->SetXY(150,100);
  $pdf->Cell(280,12,"Mérida, Yucatán, México C.P. 97259",0,0,"C");  
